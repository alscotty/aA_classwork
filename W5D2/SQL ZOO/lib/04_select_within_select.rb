...
def richer_than_england
  ...
  from 
    countries 
  where 
    (gdp/population) > (select gdp/population from countries where name
    = 'United Kingdom') and continent = 'Europe';
  SQL
end
...
def neighbors_of_certain_b_countries
  ...
  from 
    countries 
  where 
    continent in (select continent from countries where name in 
      ('Belize', 'Belgium')
    );
  SQL
end
